...
6. **(Added during Task 2 code review) `construireUrlOpenMeteo` includes `&timezone=auto`.** Without it, Open-Meteo returns `hourly.time` in UTC while `heure` is a local-time input — a mismatch would silently return the wrong real-world hour's weather instead of erroring (the whole day's 24 entries exist regardless of offset, so the lookup wouldn't miss). `timezone=auto` makes Open-Meteo resolve the timezone from the mission's own coordinates, which is also more correct than the browser's timezone. **Every occurrence of the expected URL string below (Task 1's test, Task 2's implementation) is stale by this one parameter** — read `drone-mission-app/meteo.js` and `drone-mission-app/tests/meteo.test.js` directly for the current, correct text rather than trusting the code blocks in Tasks 1-2 below.
